...
def summer():
    ...
    ambTemps = [(float(ambientData[x]) * 9/5) + 32 for x in range(len(ambientData)) if x % 2 != 0 and ambientData[x] != ""]
    ...
    fig.update_layout(title="Average Temps for Summer | GPU Internal and Outside", xaxis_title="Month", yaxis_title="Temperature (F)")
    fig.show()
